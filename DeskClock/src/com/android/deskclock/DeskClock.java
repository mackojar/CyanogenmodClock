...
public class DeskClock extends Activity implements LabelDialogFragment.TimerLabelDialogHandler,
    ...
    private int determineTargetTab(Intent intent) {
        ...
        if (intent.getComponent() != null) {
            if (intent.getComponent().getClassName().endsWith("AlarmClock")) {
    ...
    private boolean processMenuClick(MenuItem item) {
        ...
        if( R.id.menu_item_settings == itemId) {
            startActivity(new Intent(DeskClock.this, SettingsActivity.class));
            return true;
        } else if( R.id.menu_item_help == itemId) {
            ...
            return true;
        } else if( R.id.menu_item_night_mode == itemId) {
